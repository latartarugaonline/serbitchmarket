/**
 Support class for crypto algo impl and utility functions
 **/
...
{
    // Support for secure channel gen
    public static byte[] macEnc(byte[] masterKey,byte[] data) throws Exception
    {
        ...
        byte[] mid1 = desEnc(k1,data); // 40 byte
        byte[] mid2 = desDec(k2, getSub(mid1, mid1.length - 8, 8)); // 8 byte
        byte[] mid3 = desEnc(k3, getSub(mid2, 0, 8)); // 8 byte
        return mid3;
    }
